...
while ($row = mysqli_fetch_array($query)) {  // preparing an array
	...
	$img = SITE_ROOT . DS . BASE_FOLDER . UPLOAD_PATH . $ro->folder . DS . $ro->avatar;
	if (file_exists($img)) {

		if ($ro->avatar != '') {
			$img = BASE_PATH . DS . BASE_FOLDER . UPLOAD_PATH . $ro->folder . DS . $ro->avatar;
		} else {
	...
	$nestedData[] = '<strong class="text-warning fw-bolder">User:</strong>' . $ro->username . '<br><strong class="text-warning fw-bolder">Pass:</strong>' . ucfirst($ro->spass);
	$nestedData[] = ucfirst($ro->email);
	...
	$nestedData[] = '<strong class="text-info fw-bolder">C:</strong>' . datetime_to_text($ro->created_at) . '<br><strong class="text-warning fw-bolder">U:</strong>' . datetime_to_text($ro->updated_at);
